...
def exp_sen3_b(xi):
    ...
    eps = 1e-8
    C = torch.zeros(phi.size(0), 3, 3, device=phi.device)
    J = torch.zeros(phi.size(0), 3, 3, device=phi.device)
    x = torch.zeros(phi.size(0), 3, 3, device=phi.device)
    # x = torch.zeros(phi.size(0), 3, 2, device=phi.device)

    phi_norm = torch.norm(phi, dim=1, keepdim=True)
    sel = torch.squeeze(phi_norm > eps)

    phi_norm_sel = phi_norm[sel]  # not close to 0
    phi_no_sel = phi[~sel]        # close to 0

    if phi_norm_sel.size(0):
        # not close to 0  
        if xi.size(0) == 1:
            phi_norm_sel2 = phi_norm_sel[0]
            unit_phi_sel = phi / phi_norm_sel2
            unit_phi_skewed_sel = skew3_b(unit_phi_sel)
        
            s = torch.sin(phi_norm_sel2)
            c = torch.cos(phi_norm_sel2)

            J = (s / phi_norm_sel2) * torch.eye(3, 3, device=phi.device).repeat([phi_norm_sel2.size(0), 1, 1]) + \
                (1 - (s / phi_norm_sel2)) * torch.bmm(unit_phi_sel, unit_phi_sel.transpose(-1, -2)) + \
                ((1 - c) / phi_norm_sel2) * unit_phi_skewed_sel
            C = c * torch.eye(3, 3, device=phi.device).repeat([phi_norm_sel2.size(0), 1, 1]) + \
                (1 - c) * torch.bmm(unit_phi_sel, unit_phi_sel.transpose(-1, -2)) + \
                s * unit_phi_skewed_sel
            
            x = J.bmm(xi[:, 3:].view(-1, 3, 3).transpose(-1, -2))
            # x = J.bmm(xi[:, 3:].view(-1, 2, 3).transpose(-1,-2))
  
        else:
            unit_phi_sel = phi[sel] / phi_norm_sel
            unit_phi_skewed_sel = skew3_b(unit_phi_sel)

            s = torch.sin(phi_norm_sel)
            c = torch.cos(phi_norm_sel)

            J[sel] = (s / phi_norm_sel) * torch.eye(3, 3, device=phi.device) + \
                     (1 - (s / phi_norm_sel)) * torch.bmm(unit_phi_sel, unit_phi_sel.transpose(-1, -2)) + \
                     ((1 - torch.cos(phi_norm_sel)) / phi_norm_sel) * unit_phi_skewed_sel
            C[sel] = c * torch.eye(3, 3, device=phi.device).repeat([phi_norm_sel.size(0), 1, 1]) + \
                    (1 - c) * torch.bmm(unit_phi_sel, unit_phi_sel.transpose(-1, -2)) + \
                     s * unit_phi_skewed_sel

            x[sel] = J[sel].bmm(xi[sel, 3:].view(-1, 3, 3).transpose(-1, -2))
            # x[sel] = J[sel].bmm(xi[sel, 3:].view(-1, 2, 3).transpose(-1,-2))
    ...
    if phi_no_sel.size(0):  
        ...
        if xi.size(0) == 1:
            phi_skewed_no_sel = skew3_b(phi_no_sel)
            J = torch.eye(3, 3, device=phi.device).repeat([phi_no_sel.size(0), 1, 1]) + 0.5 * phi_skewed_no_sel
            C = torch.eye(3, 3, device=phi.device).repeat([phi_no_sel.size(0), 1, 1]) + phi_skewed_no_sel
            xi = xi.squeeze(-1)
            x = J.bmm(xi[:, 3:].view(-1, 3, 3).transpose(-1,-2))
            # x = J.bmm(xi[:, 3:].view(-1, 2, 3).transpose(-1,-2))
        else:
            phi_skewed_no_sel = skew3_b(phi_no_sel)
            J[~sel] = torch.eye(3, 3, device=phi.device).repeat([phi_no_sel.size(0), 1, 1]) + 0.5 * phi_skewed_no_sel
            C[~sel] = torch.eye(3, 3, device=phi.device).repeat([phi_no_sel.size(0), 1, 1]) + phi_skewed_no_sel
            xi = xi.squeeze(-1)
            x[~sel] = J[~sel].bmm(xi[~sel, 3:].view(-1, 3, 3).transpose(-1,-2))
            # x[~sel] = J[~sel].bmm(xi[~sel, 3:].view(-1, 2, 3).transpose(-1,-2))
...
def exp_sen3_b2(xi):
    ...
    C = torch.zeros(phi.size(0), 3, 3, device=phi.device)
    J = torch.zeros(phi.size(0), 3, 3, device=phi.device)
    x = torch.zeros(phi.size(0), 3, 4, device=phi.device)
    ...
    if phi_norm_sel.size(0):
        ...
        if xi.size(0) == 1:
            ...
            J = (torch.sin(phi_norm_sel2) / phi_norm_sel2) * torch.eye(3, 3, device=phi.device) + \
                (1 - (torch.sin(phi_norm_sel2) / phi_norm_sel2)) * torch.bmm(unit_phi_sel, unit_phi_sel.transpose(-1, -2)) + \
                ((1 - torch.cos(phi_norm_sel2)) / phi_norm_sel2) * unit_phi_skewed_sel

            C = c * torch.eye(3, 3, device=phi.device).repeat([phi_norm_sel2.size(0), 1, 1]) + \
                (1 - c) * torch.bmm(unit_phi_sel, unit_phi_sel.transpose(-1, -2)) + \
                s * unit_phi_skewed_sel
            
            x = J.bmm(torch.cat((xi[:,0:3],xi[:,3:]),-2).view(-1, 4, 3).transpose(-1,-2))
            ...
            J[sel] = (torch.sin(phi_norm_sel) / phi_norm_sel) * torch.eye(3, 3, device=phi.device) + \
                     (1 - (torch.sin(phi_norm_sel) / phi_norm_sel)) * torch.bmm(unit_phi_sel, unit_phi_sel.transpose(-1, -2)) + \
                     ((1 - torch.cos(phi_norm_sel)) / phi_norm_sel) * unit_phi_skewed_sel

            C[sel] = c * torch.eye(3, 3, device=phi.device).repeat([phi_norm_sel.size(0), 1, 1]) + \
                    (1 - c) * torch.bmm(unit_phi_sel, unit_phi_sel.transpose(-1, -2)) + \
                     s * unit_phi_skewed_sel
            x[sel] = J[sel].bmm(torch.cat((xi[sel,0:3],xi[sel,3:]),-2).view(-1, 4, 3).transpose(-1,-2))
    ...
    if phi_no_sel.size(0):  
        ...
        if xi.size(0) == 1:
            ...
            C = torch.eye(3, 3, device=phi.device).repeat([phi_no_sel.size(0), 1, 1]) + phi_skewed_no_sel
            xi = xi.squeeze(-1)
            x = J.bmm(torch.cat((xi[:,0:3].unsqueeze(-1),xi[:,3:].unsqueeze(-1)),-2).view(-1, 4, 3).transpose(-1,-2))
        else:
            phi_skewed_no_sel = skew3_b(phi_no_sel)
            J[~sel] = torch.eye(3, 3, device=phi.device).repeat([phi_no_sel.size(0), 1, 1]) + 0.5 * phi_skewed_no_sel
            C[~sel] = torch.eye(3, 3, device=phi.device).repeat([phi_no_sel.size(0), 1, 1]) + phi_skewed_no_sel
            xi = xi.squeeze(-1)
            x[~sel] = J[~sel].bmm(torch.cat((xi[~sel,0:3].unsqueeze(-1),xi[~sel,3:].unsqueeze(-1)),-2).view(-1, 4, 3).transpose(-1,-2))
...
def J_left_SO3_b(phi):
    ...
    if phi_norm_sel.size(0):
        unit_phi_sel = phi[sel] / phi_norm_sel
        J[sel] = (torch.sin(phi_norm_sel) / phi_norm_sel) * torch.eye(3, 3, device=phi.device).repeat(phi_norm_sel.size(0), 1, 1) + \
            (1 - (torch.sin(phi_norm_sel) / phi_norm_sel)) * torch.matmul(unit_phi_sel, unit_phi_sel.transpose(-2, -1)) + \
            ((1 - torch.cos(phi_norm_sel)) / phi_norm_sel) * skew3(unit_phi_sel)

    phi_no_sel = phi[~sel]
    if phi_no_sel.size(0):
        J[~sel] = torch.eye(3, 3, device=phi.device).repeat(phi_no_sel.size(0), 1, 1) + 0.5 * skew3_b(phi_no_sel)
