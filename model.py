...
class IMUIEKF_left(nn.Module):
    STATE_VECTOR_DIM = 18

    def __init__(self):
        super(IMUIEKF_left, self).__init__()
    ...
    def predict_one_step(self, t_accum, C_accum, r_accum, v_accum, dt, g_k, v_k, bw_k, ba_k, covar,
        ...
        v = v_k - g_k * t_accum + v_accum    # v_bk_btau_i
        
        # 构建尺寸为[batch_size, 1, 1]的单位对角三维矩阵
        I3 = torch.eye(3, 3, device=covar.device).repeat(batch_size, 1, 1)
        exp_int_w = torch_se3.exp_SO3_b(dt * w)

        r = v_k * t_accum - 0.5 * g_k * t_accum * t_accum + r_accum   # r_bk_btau_bk

        w_skewed = torch_se3.skew3_b(w)
        a_g_skewed = torch_se3.skew3_b(a_g)

        F = torch.zeros(batch_size, 18, 18, device=covar.device)
        F[:, 0:3, 0:3] = -w_skewed
        F[:, 0:3, 12:15] = -I3
        F[:, 3:6, 0:3] = -a_g_skewed
        F[:, 3:6, 0:3] = -w_skewed
        F[:, 3:6, 9:12] = -I3
        F[:, 3:6, 15:18] = -I3
        F[:, 6:9, 3:6] = I3
        ...
        Phi = torch.eye(18, 18, device=covar.device).repeat(batch_size, 1, 1) + \
              F * dt + 0.5 * mm(F, F) * dt2 + 1.0 / 6.0 * mm(mm(F, F), F) * dt3
    ...
    def predict(self, imu_meas, imu_noise_covar, prev_state, prev_covar):
        ...
        U = torch.diag(torch.tensor([0.] * 3 + [1.] * 3 + [0.] * 3 + [1.] * 3 + [1.] * 3 + [1.] * 3, device=imu_meas.device)).repeat(num_batches, 1, 1)
        pred_covar = torch.matmul(torch.matmul(U, prev_covar), U.transpose(-2, -1))

        # 一步预测states列表，汇总汇总t_k到t_k+1之间每次一步预测计算的g、C、r、v、bw、ba的一步预测值
        pred_states = []
        # 一步预测协方差列表，汇总t_k到t_k+1之间每次一步预测计算的协方差矩阵
        pred_covars = []

        g_k, _, _, v_k, bw_k, ba_k = IMUKalmanFilter.decode_state_b(prev_state)
        # 遍历t_k时刻到t_k+1时刻的所有IMU数据，并基于此进行一步预测
        for tau in range(0, imu_meas.size(1) - 1):
            ...
            # 执行一步预测，返回累积时间、累积一步预测旋转矩阵、累积一步预测平移量、累积一步预测线速度、一步预测协方差阵
    ...
    def meas_residual_and_jacobi(self, C_pred, r_pred, vis_meas):
        ...
        mm = torch.matmul
        I3 = torch.eye(3, 3).repeat(batch_size, 1, 1)
        ...
        residual_trans = vis_meas_trans - r_pred

        # 测量残差
        residual = torch.cat([residual_rot, residual_trans], dim=1)

        H = torch.zeros(vis_meas.shape[0], 6, 18, device=vis_meas.device)
        H[:, 0:3, 0:3] = torch_se3.J_left_SO3_inv_b(-phi_pred)
        H[:, 3:6, 6:9] = C_pred

        return residual, H
    
    def meas_inno_and_jacobi(self, C_pred, r_pred, vis_meas, vis_meas_covar):
        batch_size = C_pred.size(0)
        ...
        # 新息=观测-预测
        phi_pred = torch_se3.log_SO3_b(C_pred)
        inno_rot = vis_meas_rot - phi_pred
        inno_trans = vis_meas_trans - r_pred
        inno = torch.cat([inno_rot, inno_trans], dim=1)

        H = torch.zeros(vis_meas.shape[0], 6, 18, device=vis_meas.device)
        H[:, 0:3, 0:3] = I3
        H[:, 3:6, 6:9] = C_pred

        S = torch.zeros(vis_meas.shape[0], 6, 6, device=vis_meas.device)
        S[:, 0:3, 0:3] = torch_se3.J_left_SO3_b(-phi_pred)
        S[:, 3:6, 3:6] = I3
        R = mm(mm(S, vis_meas_covar), S.transpose(-2, -1))
    ...
    def update(self, pred_state, pred_covar, vis_meas, vis_meas_covar, T_imu_cam):
        ...
        if inno_based:
        # 计算新息和测量左雅克比矩阵
            inno, H, R = self.meas_inno_and_jacobi(C_pred, r_pred, vis_meas, vis_meas_covar)
        else:
        # 计算测量残差和测量左雅克比矩阵
            inno, H = self.meas_residual_and_jacobi(C_pred, r_pred, vis_meas)
            R = vis_meas_covar

        H = -H
        H_transpose = H.transpose(-2, -1)

        S = mm(mm(H, pred_covar), H_transpose) + R
        K = mm(mm(pred_covar, H_transpose), S.inverse())
        est_error = mm(K, inno)
        dC, dxi = torch_se3.exp_sen3_b(est_error[:,:12])

        I18 = torch.eye(18, 18, device=pred_state.device).repeat(vis_meas.size(0), 1, 1)
        est_covar = mm(I18 - mm(K, H), pred_covar)

        dv = dxi[:, :, 0].unsqueeze(-1)
        dr = dxi[:, :, 1].unsqueeze(-1)
        dg = dxi[:, :, 2].unsqueeze(-1)
        dC_transpose = dC.transpose(-2,-1)
        bw_err = est_error[:, 12:15]
        ba_err = est_error[:, 15:18]

        C_up = mm(C_pred, dC_transpose).contiguous()
        v_up = v_pred - mm(C_up, dv).contiguous()
        r_up = r_pred - mm(C_up, dr).contiguous()
        g_up = g_pred - mm(C_up, dg).contiguous()
        bw_up = bw_pred - bw_err
        ba_up = ba_pred - ba_err

        est_state = IMUKalmanFilter.encode_state_b(g_up,
                                                   C_up,
                                                   r_up,
                                                   v_up,
                                                   bw_up,
    ...
    def composition(self, prev_pose, pred_state, est_state, est_covar):
        batch_size = est_state.size(0)

        mm = torch.matmul
        I3 = torch.eye(3, 3, device=prev_pose.device).repeat(batch_size, 1, 1)
        ...
        new_pose[:, 0:3, 0:3] = mm(C_transpose, prev_pose[:, 0:3, 0:3])
        new_pose[:, 0:3, 3:4] = mm(C_transpose, prev_pose[:, 0:3, 3:4] - r)

        new_v = mm(C_transpose, v)
        new_g = mm(C_transpose, g)

        new_state = IMUKalmanFilter.encode_state_b(new_g, C, r, new_v, bw, ba)

        U = torch.diag(torch.tensor([0.] * 3 + [1.] * 3 + [0.] * 3 + [1.] * 3 + [1.] * 3 + [1.] * 3, device=prev_pose.device)).repeat(batch_size, 1, 1)
    ...
    def forward(self, imu_data, imu_noise_covar,
        ...
        for k in range(0, num_timesteps):
            ...
            new_pose, new_state, new_covar = self.composition(poses_over_timesteps[-1], pred_states[-1], est_state, est_covar)
...
class IMUIEKF(nn.Module):
    STATE_VECTOR_DIM = 18

    def __init__(self):
        super(IMUIEKF, self).__init__()
    ...
    def predict_one_step(self, t_accum, C_accum, r_accum, v_accum, dt, g_k, v_k, bw_k, ba_k, covar,
        ...
        r_skewed = torch_se3.skew3_b(r)
        g_skewed = torch_se3.skew3_b(g_k)

        F = torch.zeros(batch_size, 18, 18, device=covar.device)
        F[:, 0:3, 12:15] = -C_accum
        F[:, 3:6, 9:12] = -I3
        F[:, 3:6, 12:15] = -mm(v_skewed, C_accum)
        F[:, 3:6, 15:18] = -C_accum
        F[:, 6:9, 3:6] = I3
        F[:, 6:9, 12:15] = -mm(r_skewed, C_accum)
        F[:, 9:12, 12:15] = -mm(g_skewed, C_accum)

        G = torch.zeros(batch_size, 18, 12, device=covar.device)
        G[:, 0:3, 0:3] = C_accum
        G[:, 3:6, 0:3] = mm(v_skewed, C_accum)
        G[:, 3:6, 3:6] = C_accum
        G[:, 6:9, 0:3] = mm(r_skewed, C_accum)
        G[:, 9:12, 0:3] = mm(g_skewed, C_accum)
        G[:, 12:15, 6:9] = I3
        G[:, 15:18, 9:12] = I3
    ...
    def meas_residual_and_jacobi(self, C_pred, r_pred, vis_meas, T_imu_cam):
        ...
        r_skewed = torch_se3.skew3_b(r_pred)
        ...
        # 测量左雅克比阵计算，公式见report.pdf P4公式(14)，实际上的实现和论文描述不太一样
        H = torch.zeros(vis_meas.shape[0], 6, 18, device=vis_meas.device)
        H[:, 0:3, 0:3] = torch_se3.J_left_SO3_inv_b(phi_pred)
        H[:, 3:6, 0:3] = -r_skewed
        H[:, 3:6, 6:9] = I3
    ...
    def update(self, pred_state, pred_covar, vis_meas, vis_meas_covar, T_imu_cam):
        ...
        C_up = mm(dC_transpose, C_pred).contiguous()
        v_up = mm(dC_transpose, v_pred - dv).contiguous()
        r_up = mm(dC_transpose, r_pred - dr).contiguous()
        g_up = mm(dC_transpose, g_pred - dg).contiguous()
        bw_up = bw_pred - bw_err
        ba_up = ba_pred - ba_err
    ...
    def composition(self, prev_pose, pred_state, est_state, est_covar):
        ...
        g_pred, C_pred, r_pred, v_pred, bw_pred, ba_pred = IMUKalmanFilter.decode_state_b(pred_state)
        C_pred_transpose = C_pred.transpose(-2, -1)
        v_pred_skewed = torch_se3.skew3_b(v_pred)
        g_pred_skewed = torch_se3.skew3_b(g_pred)
        ...
        new_v = torch.matmul(C_transpose, v)

        new_state = IMUKalmanFilter.encode_state_b(new_g, C, r, new_v, bw, ba)

        U = torch.zeros(batch_size, 18, 18, device=prev_pose.device)
        U[:, 3:6, 3:6] = C_pred_transpose
        U[:, 9:12, 9:12] = C_pred_transpose
        U[:, 12:15, 12:15] = I3
        U[:, 15:18, 15:18] = I3
...
class IMUKalmanFilter(nn.Module):
    ...
    def predict_one_step(self, t_accum, C_accum, r_accum, v_accum, dt, g_k, v_k, bw_k, ba_k, covar,
        ...
        Phi = torch.eye(18, 18, device=covar.device).repeat(batch_size, 1, 1) + \
              F * dt + 0.5 * mm(F, F) * dt2
        Phi[:, 6:9, 12:15] = torch.zeros(3, 3, device=covar.device) 
...
class E2EVIO(nn.Module):
    ...
    def forward(self, images, imu_data, prev_lstm_states, prev_pose, prev_state, prev_covar, T_imu_cam):
        ...
        for k in range(0, num_timesteps):
            ...
            new_pose, new_state, new_covar = self.ekf_module.composition(poses_over_timesteps[-1], pred_states[-1],
                                                                         est_state, est_covar)
